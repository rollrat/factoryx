...
- [x] 파이프 내부 체적과 유체 네트워크의 첫 예약·유체 종류 잠금 해제 조건 확정
- [x] 설비 상태별 `activeMW/idleMW/0MW` 요청 규칙, 도크의 32MW 시작 대기 래치와 차단·복구 순서 확정
- [x] 동일 우선순위 전력 구역의 결정적 차단 순서와 `powerGridId` 런타임 파생 캐시 규칙 확정
- [x] `MachineMetric` 기반 공장 현황 집계, 일부 설비만 고장 난 경우의 배지 규칙과 0수요 상태 판정 보강
- [x] 발전·송전 설비를 포함한 나머지 설비의 포트 좌표, 서비스 면, 정지 자세와 LOD 설계 카드 완성
- [x] 4단계 계약의 목표 플레이 시간, 단위별 환산 계수와 전체 반복 건설 비용 밸런스 패스
- [x] 실제 스튜디오 블록아웃으로 0·90·180·270도 포트 연결, 1인칭 시야와 웹 성능 예산 시각 QA

구현 근거는 각각 `WorldProductionSimulation`의 연결 유체 잠금, `PhysicalPowerRuntime`과 결정적 전력 회귀 테스트, 60초 `ProductionMetricCollector`, 정의 기반 설비 모델 감사, `calculateProjectProductionPlan`, 그리고 `/studio`의 전체 설비·회전·LOD·1인칭 검사 도구에 연결한다. 수치 계약은 자동 회귀 테스트로 고정하며 텍스처와 배포 작업은 이 문서 범위에 포함하지 않는다.
