...
현재 진행 대상은 **P0과 P1**이다. P2 이후는 문서와 인터페이스 초안까지만 준비하고 P1 형태 승인 전에 대규모 구현하지 않는다.
...
- 완료: Blender 절벽 직선·외곽 코너·자연 아치 프로토타입 3종과 LOD·충돌·소켓 검증
- 완료: 절벽 직선 5개·외곽 코너 1개·차량 회랑 자연 아치 1개의 결정적 런타임 배치와 GLB 레지스트리 연결
- 검수 결과: 높이장 절벽면을 메시 키트가 덮어 수직 규모와 층리가 읽히며, 자연 아치의 10.8m 통과 공간이 실제 화면에 노출된다.
- TODO: 직선–아치 접합부 전용 전이 모듈, 낙석·절벽 기부 세트, 절벽 메시 충돌의 플레이어 이동계 연결, 반복 실루엣 파손 변형

즉, **P0 검수 기반, P1-A/B/C 회색 블록아웃과 P4 첫 런타임 통합이 구현됐다.** 다음 형태 승인에서는 단층 반복감과 아치 접합부를 우선 판단한다.
