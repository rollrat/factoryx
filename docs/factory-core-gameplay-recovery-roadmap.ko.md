...
- [ ] 프리뷰에 실제 포트 마커와 연결 예정선 표시
- [ ] 코어·프로젝트 도크 같은 preplaced unique 설비의 통합 선택 identity
- [ ] 전력 케이블 전용 조준 포트 선택과 케이블 미리보기
- [ ] 브라우저 플레이스루 기반 시각 회귀와 온보딩 단계 완성

이 체크포인트는 완료된 기반 작업과 아직 남은 플레이스루 작업을 분리한다. 아래 단계 순서와 완료 조건은 계속 최종 기준으로 사용한다.
