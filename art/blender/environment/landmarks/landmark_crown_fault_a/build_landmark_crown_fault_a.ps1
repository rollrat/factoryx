...
& $BlenderPath --background --factory-startup --python (Join-Path $AssetDir "build_landmark_crown_fault_a.py") --python-exit-code 1 -- --source $Source --preview $Preview --asset-id $AssetId
...
# The common manifest validator writes shared state, so retain an asset-local check.
